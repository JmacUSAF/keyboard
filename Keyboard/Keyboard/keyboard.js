...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		alertTxt.text = "Press generate code to save data or import code to load previous progress";
		...
		inputElement.style.textAlign = "center"; // Center align the text
		inputElement.style.width = "550px"; // Adjust width as needed
		inputElement.style.height = "35px"; // Adjust height as needed
		inputElement.style.bottom = "-200px"; // Position relative to the canvas
		inputElement.style.left = "325px"; // Position relative to the canvas
		inputElement.style.resize = "none"; // Prevent resizing of the textarea
		document.body.appendChild(inputElement);
		
		// Add placeholder text to indicate it's an input field
		inputElement.placeholder = "Enter code here...";
		
		//--------------------------------------------------Generate button functions ---------------------------------------//
		
		// Add an event listener to the button "generateBtn"
		this.genBtn.on("click", function() {
		    let code = ""; // Initialize an empty string to store the code
		
		    // Loop through "key0" to "key59"
		    for (let i = 0; i < 60; i++) {
		        // Construct the key name dynamically
		        let keyName = "key" + i;
		
		        // Check if the key exists on the main timeline
		        if (this[keyName]) {
		            // Add "1" if isPressed is true, otherwise add "0"
		            code += this[keyName].isPressed ? "1" : "0";
		        } else {
		            // If the key doesn't exist, add "0" to maintain code length
		            code += "0";
		            console.warn(`${keyName} does not exist on the main timeline.`);
		        }
		    }
		
		    // Log the final 60-character code
		    console.log(`Generated Code: ${code}`);
		
		    // Update the input element with the generated code
		    inputElement.value = code;
			//update alert textfield
			alertTxt.text = "Copy this code to save the class' progress"
		
		}.bind(this)); // Bind "this" to ensure the correct context is maintained
		
		//--------------------------------------------------import button functions ---------------------------------------//
		// Add an event listener to the "importBtn"
		this.importBtn.on("click", function() {
		    // Get the 60-character code string from the input field
		    const code = inputElement.value;
		
		    // Ensure the code is exactly 60 characters long
		    if (code.length !== 60) {
		        alertTxt.text = "Error: Code must be exactly 60 characters long";
		        return;
		    }
		    ...
		    for (let i = 0; i < 60; i++) {
		        ...
		        if (this[keyName]) {
					...
					this[keyName].isPressed = false;
					this[keyName].gotoAndStop("up");
					
					if(code.charAt(i) === "1"){
						this[keyName].isPressed = true;
						this[keyName].gotoAndStop("down");
						pressedKeys++;
					}
		        } else {
		            console.warn(`${keyName} does not exist on the main timeline.`);
			...
			calculatePressedPercentage(pressedKeys, NUM_KEYS);
			xPos = xPos + (pressedKeys * 10);
			scrubber.x = xPos;
			alertTxt.text = "Code has been successfully loaded"
		    console.log("Keys have been updated based on the imported code.");
	...
	this.importBtn.setTransform(680,1019.8,1,1,0,0,0,59.5,24.5);

	this.genBtn = new lib.mc_genBtn();
	this.genBtn.name = "genBtn";
	this.genBtn.setTransform(520,1019.8,1,1,0,0,0,59.5,24.5);

	this.alertTxt = new cjs.Text("", "bold 24px 'Burbank Big Cd Bk'", "#CC0000");
	this.alertTxt.name = "alertTxt";
	this.alertTxt.textAlign = "center";
	this.alertTxt.lineHeight = 26;
	this.alertTxt.lineWidth = 666;
	this.alertTxt.parent = this;
	this.alertTxt.setTransform(610.75,1127.55);
	...
	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_1},{t:this.teacherTxt},{t:this.instance},{t:this.alertTxt},{t:this.genBtn},{t:this.importBtn},{t:this.myCss}]}).wait(1));
...
p.nominalBounds = new cjs.Rectangle(772.7,546.6,416.89999999999986,610.9999999999999);
// library properties:
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/keyboard_atlas_1.png?1736425119895", id:"keyboard_atlas_1"},
		{src:"https://code.jquery.com/jquery-3.4.1.min.js?1736425119912", id:"lib/jquery-3.4.1.min.js"},
		{src:"components/sdk/anwidget.js?1736425119912", id:"sdk/anwidget.js"},
		{src:"components/ui/src/css.js?1736425119912", id:"an.CSS"}
	],
	preloads: []
};
